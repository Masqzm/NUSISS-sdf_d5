...
public class GameOfLife {
    ...
    private char[][] initializeBoard(int width, int height) {
        ...
        // Add blank char array for each row to the board [from solution]
        // for (int y = 0; y < height; y++)
        //     board[y] = Constants.BLANK.substring(0, width).toCharArray();

        // Add blank char array for each row to the board [more dynamic way]
        for (int y = 0; y < height; y++)
            board[y] = " ".repeat(width).toCharArray();
